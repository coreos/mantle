...
func (op *Operation) replace(dst *os.File, src io.Reader) error {
	...
	for _, extent := range op.Operation.DstExtents {
		...
		// update_engine only writes as much of an extent as it has
		// data for, allowing extents to be defined larger than they
		// actually should be. For files this is normal for the final
		// block to be partially complete since block alignment is not
		// required and the extent can't represent partial blocks.
		//
		// BUG: For partitions block alignment is required so the
		// extent size should be correct but payloads created by
		// update_engine always set num_blocks to the chunk size, 256.
		// payloads generated by update/generator shouldn't do this.
